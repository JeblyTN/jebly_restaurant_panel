...
<script>
    ...
    if (authRole === 'employee') {  
        ...
        database.collection('vendors')
            ...
            .then(function(snapshot) {
                if (!snapshot.empty) {
                    var vendorData = snapshot.docs[0].data();
                    var authorId = vendorData.author;  
                    database.collection('withdraw_method').where('userId', '==', authorId).get().then(async function (Snapshot) {
                        if (Snapshot.docs.length > 0) {
                            checkAlreadyExist = true;
                            var data = Snapshot.docs[0].data();
                            existId = data.id;
    ...
    $(document).on("click", "a[name='setup_btn']", async function (e) {
        ...
        if (paymentMethod == 'Stripe') {
            ...
            try {
                ...
                let userRef = await database.collection('users').doc(vendorUserId).get();
                ...
                let response = await $.ajax({
                    ...
                    url: "{{ route('setup.stripe.account') }}",
                    method: 'POST',
                    data: { user_email: userData.email },
                });

                if(response.status == 'success') {
                    var stripeObject = { name: 'Stripe', enable: true, accountId: response.account_id }
                    var id = checkAlreadyExist ? existId : database.collection('tmp').doc().id;
                    if(checkAlreadyExist) {
                        await database.collection('withdraw_method').doc(id).update({ stripe: stripeObject });
                    } else {
                        await database.collection('withdraw_method').doc(id).set({ stripe: stripeObject, id: id, userId: vendorUserId });
                    }
                ...
                if (err.responseJSON) {
                    console.error('Laravel Error:', err.responseJSON);
                    alert('Failed: ' + JSON.stringify(err.responseJSON.message || err.responseJSON));
                } else if (err.responseText) {
                    console.error('Laravel Error Text:', err.responseText);
                    alert('Failed: ' + err.responseText);
                } else {
                    console.error('Stripe Setup Error:', err);
                    alert('Failed to setup Stripe account.');
                }
                jQuery("#data-table_processing").hide();
                return false;
            }
        ...
        if (methodName == 'Stripe') {
            ...
            if (accountId == '') {
                ...
                (database.collection('withdraw_method').doc(id).set({
                    'stripe': stripeObject,
                    'id': id,
                    'userId': (authRole == 'vendor') ? vendorUserId : empVendorId
                }).then(function (result) {
                    window.location.href = '{{ route("withdraw-method")}}';
                })
                )
