...
<script type="text/javascript">
    ...
    async function buildHTML(val) {
        ...
        const expireDate = new Date(val.expiresAt.toDate()); 
        const currentDate = new Date();
        const isExpired = expireDate < currentDate;
        route1 = route1.replace(':id', id);
        if (currentPermissions.isDelete) {
        ...
        if (val.isEnabled) {  
            html.push('<td><label class="switch"><input type="checkbox"' + (isExpired ? 'disabled' : 'checked') + ' id="' + val.id + '" name="isEnabled"><span class="slider round"></span></label></td>');
        } else { 
            html.push('<td><label class="switch"><input type="checkbox"' + (isExpired ? 'disabled' : 'checked') + ' id="' + val.id + '" name="isEnabled"><span class="slider round"></span></label></td>');
        }
        ...
        html.push(
            ...
            + (currentPermissions?.isDelete
                ? '<a id="' + val.id + '" name="coupon_delete_btn" class="do_not_delete" href="javascript:void(0)"><i class="fa fa-trash"></i></a>'
                : '')
            + '</span>'
        );
    ...
    async function getVendorId(vendorUser) {
      var vendorId = '';
        if(authRole == 'vendor'){
            let vendorSnapshots = await database.collection('vendors').where('author', '==', vendorUser).get();
            if (!vendorSnapshots.empty) {
                let vendorData = vendorSnapshots.docs[0].data();
                vendorId = vendorSnapshots.docs[0].id;
            }
        }else{
            let vendorSnapshots = await database.collection('vendors').where('id', '==', empVendorId).get();
            if (!vendorSnapshots.empty) {
                let vendorData = vendorSnapshots.docs[0].data();
                vendorId = vendorSnapshots.docs[0].id;
            }
