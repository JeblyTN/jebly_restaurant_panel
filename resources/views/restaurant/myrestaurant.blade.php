                        ...
                        <div class="row restaurant_payout_create">
                            <div class="restaurant_payout_create-inner">
                                <fieldset>
                                    ...
                                    <div class="form-group row">
                                        ...
                                        <div class="form-text text-muted ml-3">
                                            {{trans('lang.know_your_cordinates')}} <a target="_blank" href="https://www.latlong.net/">{{trans('lang.latitude_and_longitude_finder')}}</a>
                                        </div>
                                    </div>
                                    <div class="form-group row">
                                ...
                                <fieldset id="story_upload_div" style="display: none;">
                                    <legend>Story</legend>
                                    <div class="form-group row">
                                        <label class="col-12 control-label" style="color:red;font-size:15px;">
                                            {{trans('lang.please_click_on_save_button_after_making_changes_in_image_or_video')}} </label>
                                    </div>
...
<script src="https://cdnjs.cloudflare.com/ajax/libs/compressorjs/1.1.1/compressor.min.js" integrity="sha512-VaRptAfSxXFAv+vx33XixtIVT9A/9unb1Q8fp63y1ljF+Sbka+eMJWoDAArdm7jOYuLQHVx5v60TQ+t3EA8weA==" crossorigin="anonymous" referrerpolicy="no-referrer"></script>
    ...
    <script>
        ...
        restaurant.get().then(async function(snapshots) {
            ...
            if (restaurantdata.packagingChargeEnable) {
        ...
        if (authRole === 'vendor') {  
            ...
            database.collection('vendors').where('id', "==", empVendorId).get().then(async function(vendorSnapshots) {
                var vendorData = vendorSnapshots && vendorSnapshots.docs && vendorSnapshots.docs.length > 0 && vendorSnapshots.docs[0].data() ? vendorSnapshots.docs[0].data() : '';
                if (vendorData) {
                    vendorId = vendorData.id;
                    const perm = await getEmployeePermissionForTitle(vendorUserId, "Working Hours");                  

                    if (perm.isActive) {
                        $('#working-hour-div').removeClass('d-none');
                    } else{
                        $('#working-hour-div').addClass('d-none');
                    }  
                    vendorexist = true;
                }
            }) 
        ...
        document.addEventListener("DOMContentLoaded", async function() {    
            if (authRole === 'employee') {               
                const perm = await getEmployeePermissionForTitle(vendorUserId, "Restaurant Information's");
                currentPermissions = {
                ...
                if (permDine.isActive || permDineAdd.isActive) {
                    $('.dineInFuture').show(); 
                } else{
                    $('.dineInFuture').hide();
                }  
            ...
            $(document).ready(function() {
                ...
                ref.get().then(async function(snapshots) {
                    try {
                        ...
                        var menuCardPhotos = ''
                        if (restaurant.hasOwnProperty('restaurantMenuPhotos') && restaurant.restaurantMenuPhotos!=null) {
                        ...
                        if (menuCardPhotos != "" && menuCardPhotos != null) {
                            ...
                            $("#photos_menu_card").html('<p>{{trans("lang.menu_card_photos_not_available")}}</p>');
                        }
                        ...
                        if (photos != "" && photos != null) {
                            ...
                            $("#photos").html('<p>{{trans("lang.photos_not_available")}}</p>');
                        }
                                                ...
                                                if (restaurant.specialDiscount[i]['timeslot'][j]) {
                                                    ...
                                                    if (timeslot['discount']) {
                                                        ...
                                                        $('#special_offer_table_' + day + ' tr:last').after('<tr>' +
                                                            ...
                                                            '<td style="width:30% !important;"><select id="type' + day + j + i + '" class="form-control ' + i + '_' + j + '_row" ><option value="delivery">{{trans("lang.delivery_discount")}}</option><option value="dinein">{{trans("lang.dineIn_discount")}}</option></select>' +
                                                            '</td>' +
                    ...
                    if (packagingChargeEnable) {
                        ...
                        $('#packagingCharge').val(restaurant.packagingCharge);
                    }
        ...
        $(".save_restaurant_btn").click(async function() {
            if (userVerificationEnable) {
                ...
                $("#field_error").html("{{trans('lang.user_verification_pending')}}");
            }
        })
        ...
        function handleStoryFileSelect(evt) {
            ...
            video.onloadedmetadata = function() {
                ...
                if (video.duration > storevideoDuration) {
                    $(".error_top").show();
                    $(".error_top").html("");
                    $(".error_top").append("<p>{{trans('lang.story_video_duration_maximum_allow_to')}} " + story_video_duration + " {{trans('lang.seconds')}}</p>");
                    window.scrollTo(0, 0);
                    evt.target.value = '';
                    return false;
                }
        ...
        function addMoreButton(day, day2, count) {
            ...
            $('#special_offer_table_' + day + ' tr:last').after('<tr>' +
                ...
                '</td>' +
                '<td style="width:30% !important;"><select id="type' + day + count + '" class="form-control"><option value="delivery">{{trans("lang.delivery_discount")}}</option><option value="dinein">{{trans("lang.dineIn_discount")}}</option></select></td>' +
                '<td class="action-btn" style="width:20%;">' +
        ...
        function addMoreFunctionButton(day1, day2, count) {
            ...
            if (openTime == '') {
                $(".error_top").show();
                $(".error_top").html("");
                $(".error_top").append("<p>{{trans('lang.please_enter_special_offer_open_time')}}</p>");
                window.scrollTo(0, 0);
            } else if (closeTime == '') {
                $(".error_top").show();
                $(".error_top").html("");
                ...
                if (isDuplicate) {
                    $(".error_top").show();
                    $(".error_top").html("");
                    $(".error_top").append("<p>{{trans('lang.this_time_slot_for')}} " + type + " {{trans('lang.already_exists_please_select_different_time_slot')}}</p>");
                    window.scrollTo(0, 0);
                } else {
                    var timeslotVar = {
                        'discount': discount,
