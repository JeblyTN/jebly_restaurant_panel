...
<script>
    ...
    var refData = database.collection('wallet').where('user_id','==',vendorUserId).orderBy('date','desc');
    ...
    async function buildHTML(val) {
        ...
        var amount = '';
            ...
            if(val.order_id){
                ...
                route1 = route1.replace(':id', val.order_id);
                html.push('<td><a href="'+route1+'">' + val.order_id + '</a></td>');
            }else{
                html.push('<td></td>');
            }
            ...
            if (val.payment_method) {
                 if (val.payment_method == "Wallet" || val.payment_method=="wallet") {
                    ...
                    payment_method = val.payment_method == 'tax' ? '' : val.payment_method;
                }
            }
            ...
            if (val.payment_status == 'success') {
                html.push('<td><span class="order_completed">' + val.payment_status + '</span></td>');
            } else if (val.payment_status == 'undefined') {
                html.push('<td><span class="order_rejected">' + val.payment_status + '</span></td>');
