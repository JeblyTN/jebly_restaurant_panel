...
document.addEventListener("DOMContentLoaded", async function() {
    if (authRole === 'employee') {               
        ...
        ref = database.collection('restaurant_orders').orderBy('createdAt', 'desc').where('vendor.author',"==",authorId).where('status','==','Order Accepted');
        refData = database.collection('restaurant_orders').where('isPosOrder','==',false).where('vendor.author',"==",authorId).where('status','==','Order Accepted');

    }else{
        ref = database.collection('restaurant_orders').orderBy('createdAt', 'desc').where('vendor.author',"==",user_id).where('status','==','Order Accepted');
        refData = database.collection('restaurant_orders').where('isPosOrder','==',false).where('vendor.author',"==",user_id).where('status','==','Order Accepted');
    }
    ...
    async function buildHTML(val){
        ...
        html.push('<td><div data-url="'+route1+'" class="redirecttopage" style="cursor: pointer;">'+val.id+'</div></td>');
