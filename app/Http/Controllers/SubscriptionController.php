...
class SubscriptionController extends Controller
{
    ...
    public function checkout($id)
    {
        ...
        $userId = $exist->uuid;
        $planId = $id;
        // Store plan ID in session for the callback to read later
        session(['pending_plan_id' => $planId]);
    ...
    public function success()
    {
        ...
        if (isset($_GET['orangepay_token'])) {
            $orangepay_token = Session::get('orangepay_payment_token'); // your original order_id
            $fail_url = route('subscription-plan.show');
            if ($orangepay_token === $_GET['orangepay_token']) {
                ...
                if (! $orangepay_token || ! $orangepay_access_token) {
                    return response()->json(['error' => 'Missing tokens for status check']);
                }

                $url = $orangepay_isSandbox
                ...
                try {
                    ...
                    if (isset($responseBody['status']) && strtoupper($responseBody['status']) === 'SUCCESS') {
                        $cart['payment_status'] = true;
                        Session::put('cart', $cart);
                        Session::put('success', 'Payment successful');
                        Session::save();
                    } else {                        
                        return redirect($fail_url); // or show error page
                    }
                } catch (\Exception $e) {                   
                    \Log::error('OrangePay status check failed: ' . $e->getMessage());
        ...
        if (isset($_GET['flutterwave_token'])) {
        ...
        if(isset($_GET['payment_type']) && $_GET['payment_type'] == 'free'){
                $cart['payment_status'] = true;
                Session::put('cart', $cart);    
                Session::put('success', 'Payment successful');
                Session::save();
